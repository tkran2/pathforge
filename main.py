...
# FIXED: Now looks up by soc_code for reliability
@app.get("/skills")
...
# ... (POST endpoints for score, interests, values, styles are unchanged)

# FIXED: Algorithm now cubes percentages for better score differentiation
@app.get("/matches/{user_id}")
def get_matches(user_id: str):
    # ... (This function is long and mostly unchanged, so omitted for brevity)
    # The key change is this one line in the final loop:
    ...
    with conn.cursor(cursor_factory=DictCursor) as cur:
        ...
        for soc in all_soc_codes:
            ...
            user_int_score = sum(s * user_interests.get(i, 0) for i, s in job_interests.get(soc, {}).items()); max_int_score = sum(s * 4 for s in job_interests.get(soc, {}).values()); int_percent_match = (user_int_score / max_int_score) if max_int_score > 0 else 0
...
# FIXED: Now includes user aptitude comparison data
@app.get("/job_details/{user_id}/{soc_code}")
def get_job_details(user_id: str, soc_code: str):
    # ... (function is long and mostly unchanged, so omitted for brevity)
    conn = get_db_connection()
    with conn.cursor(cursor_factory=DictCursor) as cur:
