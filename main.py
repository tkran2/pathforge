...
def get_demand(role: str, region: Optional[str] = None):
    # ... (function is unchanged, omitted for brevity)
...
def get_matches(user_id: str):
    ...
    with conn.cursor(cursor_factory=DictCursor) as cur:
        ...
        for soc in all_soc_codes:
            ...
            user_val_score = sum(s * user_values.get(v, 0) for v, s in job_values.get(soc, {}).items()); max_val_score = sum(s for s in job_values.get(soc, {}).values()); val_percent_match = (user_val_score / max_val_score) if max_val_score > 0 else 0
